...
Локальный HTTP-сервис для приема CommerceML-обмена из 1С. Сервис поднимает endpoints `/exchange` и `/1c_exchange.php`, проверяет Basic Auth, принимает файлы выгрузки и сохраняет их в локальную директорию.
...
| `COMMERCE_FILE_LIMIT_BYTES` | `10485760` | Значение `file_limit`, которое сервис возвращает 1С на `mode=init`. |
| `COMMERCE_ZIP_ENABLED` | `false` | Если `true`, сервис возвращает `zip=yes` на `mode=init`; пришедшие zip-файлы сохраняются без распаковки. |
...
| `mode=init` | Возвращает `zip=no` или `zip=yes` и `file_limit`. |
...
| `type=sale&mode=query` | Возвращает пустой CommerceML-документ без заказов. |
| `type=sale&mode=success` | Подтверждает успешную загрузку заказов в 1С. |
...
Также доступен совместимый путь:

```text
http://localhost:3010/1c_exchange.php
```
